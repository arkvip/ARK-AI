...
22. 已提交：tool-runtime H4 `ToolUseContext` portable facts owner guard。已确认可外移 facts 与 runtime-only fields 的边界，继续保留 collapsed unlock state、custom data、workspace services、cancellation token、computer-use host 与 Deep Review checkpoint hook 为 core-owned runtime；若需要迁移 `ToolUseContext` owner，必须另补 port/provider 设计和旧路径兼容。
23. 已提交：tool-runtime H5 `GetToolSpec` unlock state guard。已锁定 execution 侧 `GetToolSpec` 解锁收集只接受成功的 `GetToolSpec` 结果、只输出 collapsed 白名单中的工具、并保持去重和过滤语义；本阶段仍不迁移 `GetToolSpecTool`、runtime manifest assembly、`ToolUseContext` 或 concrete tools。
24. 当前阶段：tool-runtime H6 contextual manifest owner migration。将 context-aware prompt manifest / visible-tools resolution 的通用算法、generic catalog snapshot provider、generic GetToolSpec catalog provider 和 `GetToolSpec` collapsed summary/detail helper 迁入 `bitfun-agent-tools`，core 保留 product registry snapshot access、product collapsed catalog source、`dyn Tool` / `ToolUseContext` adapter 与旧路径返回类型；本阶段仍不迁移 concrete tools、`GetToolSpecTool` 执行、collapsed unlock state 或 snapshot decorator。
25. 后续独立评估：`bitfun-core default = []`、per-product feature set、依赖版本收敛或构建收益优化；任何收益声明都需要记录 `cargo check -p bitfun-core`、workspace check 和目标 crate check 的前后数据。
